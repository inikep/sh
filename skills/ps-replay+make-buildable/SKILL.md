...
4. Choose **exactly one** of these actions:
   ...
   **If the failure does not clearly map to one of Fold/Defer/Align/Remove, choose Stop.** Do not invent a fifth action. Do not classify a whole-file replacement as "Fold" or "Align hunks." Do not classify a forward-fold from a later source-list commit as "Fold."
...
- `ps_replay_auto_loop.sh`: replay driver that cherry-picks a 1-based source-list range, buckets each commit, auto-resolves DU files absent on REFERENCE, runs `ps_replay_resolve_hunks.py` against any markered files, skips empty cherry-picks (rule 14), and runs the per-commit build for source/plugin commits (rule 9). Stops on unresolved markers, remaining unmerged files, or build failures with the log path. Does **not** auto-fix build failures (those require Fold/Defer/Align/Remove judgment per rule 6). Configured via env vars `PS_REPLAY_SRC_LIST`, `PS_REPLAY_LOG_DIR`, `PS_REPLAY_BUILD_DIR`, `PS_REPLAY_WORKTREE` (default cwd), `PS_REPLAY_REFERENCE` (default `ps-5.7.9-gca-start`), `PS_REPLAY_SCRIPTS` (default the script's own directory). Use this for the bulk of the post-Group-7 replay; switch back to direct git when a stop demands per-commit reasoning.
