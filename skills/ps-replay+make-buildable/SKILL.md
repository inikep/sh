...
Detection cross-check (mandatory before continuing any Source-bucket cherry-pick): after staging the resolved files but **before** `git cherry-pick --continue`, run `git diff --cached --name-only` and compare to the source commit's `git diff-tree --no-commit-id --name-only -r <source-sha>`. If any source/plugin/build-system path from the source commit is absent from the staged diff, the absence must be accounted for by exactly one of these ledger entries; otherwise **stop and ask the engineer**:
  ...
  - An **applied-equivalent** entry for an absent source path or hunk that is either (a) **reference-absent nonsemantic**: the path is absent from `$REFERENCE_BRANCH` and `$OUTPUT_BRANCH`, the source diff is demonstrably whitespace-only, line-ending-only, comment-only, or context-only after whitespace-ignored inspection, and no code, declaration, build rule, test assertion, or generated ABI content is being dropped; or (b) **destination/reference-equivalent semantic**: the affected paths are named, inspection shows `$OUTPUT_BRANCH` and `$REFERENCE_BRANCH` already carry the intended final behavior through a different 5.7 implementation/API/path shape, and any retained tests or fixtures from the source commit remain staged. Semantic applied-equivalents may be ledgered without a fresh engineer round-trip when the old source path/hunk is mapped to named destination/reference path(s) at hunk or symbol level and the verification commands demonstrate the equivalent behavior or intentional reference absence. Do not stop solely because the source-era semantic path moved, split, or renamed in 5.7; first perform the mapping inspection and ledger the applied-equivalent when the evidence is present. Semantic applied-equivalents may also be ledgered when the engineer approves the specific source idx/SHA or directly acknowledges the current conflict case in the current conversation. Semantic applied-equivalents include source-era changes whose 5.7 equivalent is intentionally present, moved, split, renamed, or intentionally absent in the reference, such as 5.6 `read_view_t`/`trx_sys_t` descriptor changes mapping to 5.7 `ReadView`/`MVCC`, handler result-lifecycle cleanup mapping to retained 5.7 ownership, fake-changes handlerton/API plumbing absent from 5.7, adaptive hash index partitioning mapping to the 5.7 AHI latch/hash-table shape, buffer-pool mutex split hunks mapping to 5.7 block/flush/LRU mutex ownership, or standalone LRU-manager thread plumbing mapping to the 5.7 page-cleaner coordinator/worker shape. Resolve only the conflict hunks and directly dependent call-site/test/declaration/startup/PFS hunks; do not introduce obsolete 5.6 APIs or remove 5.7 reference-retained behavior unless the reference does so; delete source-era tests only when they are absent from the reference; and record the source idx/SHA, source path(s), destination/reference path(s) or `reference-absent`, old source shape or exact nonsemantic reason, destination/reference equivalent or absence verification, and verification command/output summary in the applied-equivalents ledger and `$REPORT_FILE`. This exception does not authorize unmapped semantic drops, whole-file reference alignment, directory-wide resets, dropping unrelated behavior, skipping the HP-8 staged-path cross-check, rebucketing a Source commit as no-build, copying whole files from `$REFERENCE_BRANCH`, or skipping a required build for a non-empty Source-bucket commit.
...
10. Start a **deferred-hunks/commits/forward-folds/squashes/clusters/applied-equivalents ledger** for content whose position in the output history differs from its position in the source list. The ledger has six sections:
   ...
   - **Applied-equivalents** (no-op false-positive shielding): source-list commits whose content was applied to `$OUTPUT_BRANCH` with modifications (e.g. via forward-fold, Align, or hunk-defer at the original position), or whose absent source paths are accounted for by a bounded HP-8 exception such as reference-absent nonsemantic changes or destination/reference-equivalent semantic changes, making the post-hoc `git patch-id` of the source commit not match anything on `$OUTPUT_BRANCH`. Without this section, a still-deferred-list rebuilt by `patch-id` after the fact will list these commits as deferred, and the driver will waste cycles re-attempting them. Each entry: source idx/SHA, new SHA on `$OUTPUT_BRANCH` if a commit was produced or `no-output-sha` for empty/path-only exceptions, reason (`forward-folded` / `align-only` / `partial-hunk` / `squash-component` / `reference-absent-nonsemantic` / `destination-reference-equivalent-semantic` / `destination-reference-equivalent-moved-split`), source path(s), destination/reference path(s) or `reference-absent`, verification summary, and pointer to the ledger entry or report section that records the actual landing or exception.
...
5. **Do not normalize reference-preserved whitespace in tests or generated fixtures.** If `git diff --check` reports trailing whitespace or similar whitespace issues in a newly added or modified test/fixture/result line, compare that exact line to `$REFERENCE_BRANCH`. If the reference keeps the whitespace, preserve it and record that `diff --check` warning as intentionally reference-matching in the report/ledger instead of cleaning it up. Only clean whitespace that is not present in the reference or is newly introduced by conflict editing.
   - 5.6-to-5.7 API relocations may be ledgered as destination/reference-equivalent semantic applied-equivalents when they are mapped at symbol level and verified against the reference. Common examples include 5.6 `sql/sql_cmd.h` command-enum hunks whose 5.7 home is `include/my_sqlcommand.h`, obsolete 5.6-only `INSERT DELAYED` call sites/tests whose 5.7 reference intentionally lacks the feature, and backup-lock command/error additions that must follow the 5.7 enum or error-numbering layout instead of their 5.6 insertion point. This does not permit whole-file replacement, directory alignment, or dropping unmapped code.
6. If the file does not exist on `$REFERENCE_BRANCH`, the conflict is between a local addition and a reference-side absence. Remove the conflicting addition hunk only if hunk-level analysis confirms the reference's deletion is the correct logic. Do not delete the whole file unless the entire file is the conflicting change and the engineer has explicitly approved removal in the current conversation.
7. Stage the resolved files: `git add <resolved-files>`.
8. **HP-8 staged-paths cross-check (mandatory for every post-Group-8 Source-bucket and Plugin-only-bucket cherry-pick, whether the cherry-pick conflicted or applied cleanly).** Before continuing, compare the staged tree's touched paths to the source commit's touched paths:
   ...
   For every source/plugin/build-system path that appears in `hp8-src-paths.txt` but not in `hp8-staged-paths.txt`, the source commit's modification of that path is silently absent. These cases are acceptable:
   ...
   3. The omission is an applied-equivalent: either reference-absent nonsemantic or destination/reference-equivalent semantic. For semantic cases, either the engineer approved/acknowledged the case in the current conversation or the ledger maps the old source path/hunk to named destination/reference path(s) at hunk or symbol level; affected paths are named, retained relevant tests or fixtures are staged, and verification is recorded in the applied-equivalents ledger and `$REPORT_FILE`.
...
9. Continue the cherry-pick: `git cherry-pick --continue`.
...
- `ps_replay_batch.py`: replay a bounded 1-based commit range from a source-list file. Stops on the first conflict, preserves empty marker commits, skips empty non-marker commits, stops on the first build failure or missing required build record. Requires the exact Group 8 marker by default. Uses `--build-policy bucketed` by default: every post-Group-8 source/plugin/build-system commit is built at its resulting SHA before the next build-required commit, while post-Group-8 no-build batches are built at batch boundaries. Use `--classify-only` first to inspect bucket decisions.
...
- `ps_replay_auto_loop.sh`: compatibility wrapper around `ps_replay_batch.py` for the historical environment-variable interface. It does not implement separate replay logic. Configure with `PS_REPLAY_SRC_LIST`, `PS_REPLAY_LOG_DIR`, `PS_REPLAY_BUILD_DIR`, `PS_REPLAY_WORKTREE` (default cwd), `PS_REPLAY_REFERENCE`, optional `PS_REPLAY_REPORT_FILE`, optional `PS_REPLAY_GROUP8_MARKER`, and `PS_REPLAY_SCRIPTS` (default the script's own directory). It inherits `ps_replay_batch.py` behavior: Group 8 checkpoint build before marker preservation, post-Group-8 source/plugin HP-8 staged-path checks, immediate builds for source/plugin commits, no-build batch boundary builds, and stop-on-conflict/build-failure.
...
- The HP-8 staged-paths cross-check shows a source/plugin/build-system path from the source commit absent from the staged tree, and the absence is not accounted for by any of: (a) a fully empty cherry-pick under rule 14, (b) a deferred-hunks ledger entry with a named later target commit, (c) a forward-folded-hunks ledger entry naming an earlier target commit on `$OUTPUT_BRANCH` that already contains the absent hunks, (d) a squashed-commits ledger entry that absorbed this commit into an earlier combined commit, (e) a reference-absent nonsemantic applied-equivalent with verification recorded in the ledger and report, or (f) a destination/reference-equivalent semantic applied-equivalent with either current-conversation engineer approval or verified hunk/symbol-level mapping to named destination/reference path(s), with verification recorded in the ledger and report.
