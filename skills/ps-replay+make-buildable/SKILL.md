...
- Mass-folding REFERENCE state from later commits into a single G7 `[compilation]` fix commit so that subsequent post-G7 Source-bucket cherry-picks resolve to empty/no-op and their per-commit builds are escaped (see also HP-8 and rule 10). The G7 `[compilation]` fix commits address compile errors of the current G7 tree only; they do not pre-stage content that future commits would have introduced.
- **Forward-folding** content from a later source-list commit into the current commit's build-fix to repair a build failure. Patterns like "fold idx 334's `packaging/rpm-fedora/rpm-oel` file restorations forward into idx 330's build fix," "pull the missing header from idx N+k into idx N to make it build," or any equivalent that reaches forward in the source list for fix material are forbidden. Build-Driven Fixes (§6) reach **backward** to `$REFERENCE_BRANCH` for guidance on hunk-level edits to the current commit's own changed paths; they do not reach forward into commits that have not yet been applied. The **legitimate alternative** when reasoning toward forward-fold is `§6 Defer-commit`: move the *current commit* forward to a landing position where its dependencies have arrived, instead of pulling *future content* backward. Defer-commit respects causal order; forward-fold violates it. If neither a narrow Fold/Defer/Align/Remove nor a Defer-commit (under its strict bounds) can make the current commit buildable, that is a Stop Condition — stop and ask, per rule 2 and §6's three-attempt limit, do not silently pull future-commit content backward.
...
- Mass-folding REFERENCE content from later commits into a single G7 `[compilation]` fix commit so that subsequent Source-bucket cherry-picks become empty or no-build (see also HP-2 and rule 10 below). "Minimal G7 fix" means fixing the current G7 tree, not pre-staging future commits' content.
...
- A Source-bucket or Plugin-only-bucket post-Group-7 commit was rebucketed as no-build after resolution, had its source/plugin/build-system hunks stripped to escape its per-commit build requirement, or was subjected to an "align source paths to REFERENCE after cherry-pick" pass — including any "PROTECTED set" / "exclusion list" / "all paths except <N>" variant of that pass (HP-1, HP-8). The HP-8 staged-paths cross-check was skipped for any post-G7 Source-bucket or Plugin-only-bucket cherry-pick.
...
- A G7 `[compilation]` fix commit folded REFERENCE state from later commits, beyond the minimum required to make the current G7 tree build, so that subsequent post-G7 Source-bucket cherry-picks resolved to empty/no-op and their per-commit builds were escaped (HP-2, HP-8, rule 10). Equivalently, a post-G7 build-fix forward-folded content from a not-yet-applied later source-list commit into the failing earlier commit (HP-2 forward-fold ban, §6).
...
- Final Parity proceeded while the deferred-hunks/commits ledger still contained an unapplied entry.
...
2. Reordering is allowed **only** in two narrow forms, both of which require explicit bookkeeping:
   ...
   Record every reordered or deferred commit, its original index, its new index, and the specific conflict or build failure that motivated the change in `$REPORT_FILE`. If reordering is being considered for any other reason — convenience, throughput, "it just works better," "let me move several commits to a more convenient cluster" — do not reorder. Multi-commit reorders that are not narrow conflict-motivated reorders and that do not fit within a single Defer-commit's bounds are a Stop Condition.
...
5. When a commit does not build in isolation **after the Group 7 marker**, classify the failure before editing. The classification must be one of: missing dependency to fold, premature hunk to defer, premature whole commit to defer-forward to a specific later landing position (§6 Defer-commit), incoherent source-only addition to remove, or unresolved design issue. **If the failure does not clearly fit one of these five categories, stop and ask the engineer.** Do not invent a sixth category. Do not classify as "fold" what is actually whole-file replacement. Do not classify as "defer-commit" what is actually a multi-commit reorder (Defer-commit moves exactly one commit).
...
12. Do not rely on a later merge, final source commit, reconciliation commit, or final build to make earlier unbuildable commits coherent or to replace missing required post-Group-7 build evidence.
...
16. After all source commits are replayed and every required post-Group-7 build record is present, reconcile the tree to a null diff with `$REFERENCE_BRANCH` through explicit path/hunk-level reconciliation commits. Do not use tree snapping (HP-1).
...
9. Start a **deferred-hunks/commits ledger** for cascade-causing changes that must be applied later with their dependent commit. The ledger has two sections:
   - **Deferred hunks**: hunks removed from an earlier commit under §5's hunk-level bounds, each with source commit, file, short reason, and target later commit that will re-apply the hunk.
   - **Deferred commits**: whole commits postponed forward under §6's Defer-commit bounds, each with original index/SHA, original subject, landing index/SHA, motivating build failure, the dependency the landing commit provides, and (once applied) the new `$OUTPUT_BRANCH` SHA and build log path.

   Both sections must converge before Final Parity: every ledger entry must record the position at which the deferred content was re-applied, and Final Parity (§10) verifies the ledger is empty.
...
4. Choose **exactly one** of these actions:
   - **Fold**: add the smallest required reference or source-required declaration/member/enum entry to the current commit. "Reference" here means `$REFERENCE_BRANCH`'s tree (the backward authority for hunk-level guidance). **Forward-folding** — pulling content from a not-yet-applied later source-list commit into the current commit — is **not** a Fold action and is forbidden (HP-2).
   ...
   - **Defer-commit** (whole-commit-level): postpone the **entire** current commit to a specific later source-list position where its dependencies have arrived, instead of forcing a large speculative Fold into the current branch. Use this when the failing commit is *mostly* premature for the current build point — i.e., when the minimum Fold required to make it build would itself reach forward across many later commits and would be functionally indistinguishable from a forbidden forward-fold (HP-2). Defer-commit is the **right alternative** when you catch yourself reasoning toward forward-folding a large body of future content into the current commit; it moves the *current commit* forward instead of pulling *future content* backward.

     Bounds (every bound must hold; if any is uncertain, choose Stop):

     a. **Specific named landing position.** Identify a specific later source-list commit (by index and SHA, e.g. `idx M, <sha>`) that introduces the missing dependency. The deferred commit will be cherry-picked immediately after that landing commit's required build PASS. "Somewhere later in the post-G7 range" is not specific enough.
     ...
     Defer-commit is **not** a substitute for fixing a small/clean Fold/Defer/Align/Remove. Prefer the narrower actions first; use Defer-commit only when the alternative would be a large speculative Fold of forward content.
   ...
   **If the failure does not clearly map to one of Fold/Defer/Defer-commit/Align/Remove, choose Stop.** Do not invent a sixth action. Do not classify a whole-file replacement as "Fold" or "Align hunks." Do not classify a forward-fold from a later source-list commit as "Fold." Do not classify a multi-commit reorder as "Defer-commit" (Defer-commit moves exactly one commit; it does not cascade — see bound (b)).

5. After at most three Fold/Defer/Defer-commit/Align/Remove attempts on the same source commit (counted across **all** positions it has been tried at, including the original position before a Defer-commit and the landing position after), if the build is still failing, choose Stop. Do not continue iterating; the iteration limit exists to prevent unbounded reasoning toward forbidden actions and to prevent commits from being bounced forward indefinitely.
...
- `ps_replay_resolve_hunks.py`: hunk-level conflict-region resolver. For each `<<<<<<< / ||||||| / ======= / >>>>>>>` block in the listed files (or `--all` for every markered file), score each side's distinct non-trivial lines against the corresponding `$REFERENCE_BRANCH` file and pick the higher-overlap side; HEAD-tiebreak. Replaces only the conflict block; merged context outside markers is untouched. Exit 1 if any region was left unresolved (neither side overlaps reference); those need manual hunk-level work using `git show $REFERENCE:<path>` for inspection only. HP-1 compliant: never copies whole files. Treat the result as best-effort: if the chosen side later breaks the build, fix via Fold/Defer/Defer-commit/Align/Remove — do not loop the resolver with looser thresholds.
- `ps_replay_auto_loop.sh`: replay driver that cherry-picks a 1-based source-list range, buckets each commit, auto-resolves DU files absent on REFERENCE, runs `ps_replay_resolve_hunks.py` against any markered files, skips empty cherry-picks (rule 14), and runs the per-commit build for source/plugin commits (rule 9). Stops on unresolved markers, remaining unmerged files, or build failures with the log path. Does **not** auto-fix build failures (those require Fold/Defer/Defer-commit/Align/Remove judgment per rule 6 — in particular Defer-commit decisions, which require human reasoning about landing position and intervening-commit dependencies). Configured via env vars `PS_REPLAY_SRC_LIST`, `PS_REPLAY_LOG_DIR`, `PS_REPLAY_BUILD_DIR`, `PS_REPLAY_WORKTREE` (default cwd), `PS_REPLAY_REFERENCE` (default `ps-5.7.9-gca-start`), `PS_REPLAY_SCRIPTS` (default the script's own directory). Use this for the bulk of the post-Group-7 replay; switch back to direct git when a stop demands per-commit reasoning.
...
# failure. Inspect, fix per Fold/Defer/Defer-commit/Align/Remove, amend, re-run
# with updated START.
```
...
Also audit the **deferred-hunks/commits ledger**: every entry must record the position at which the deferred content was re-applied. Specifically, every Defer-commit entry must have a landing SHA on `$OUTPUT_BRANCH` and a PASS build log at that SHA, and every deferred-hunk entry must record the later commit that re-applied the hunk. **If any ledger entry is still unapplied, do not start Final Parity.** Stop and ask the engineer whether to apply the deferred content now (at its named landing position, if available, or via a Stop-and-ask decision) or to declare the run invalid.
...
- A commit cannot be made buildable within three Fold/Defer/Defer-commit/Align/Remove attempts (counted across all positions it has been tried at).
- A commit remains non-buildable after minimal `$REFERENCE_BRANCH` fixes, bounded hunk deferral, and (where applicable) a single Defer-commit to a specific named landing position.
...
- Final Parity is reached while the deferred-hunks/commits ledger still contains an unapplied entry.
...
- A build-driven fix does not clearly map to Fold, Defer (hunk), Defer-commit (whole-commit), Align, or Remove.
...
- The HP-8 staged-paths cross-check shows a source/plugin/build-system path from the source commit absent from the staged tree, and the absence is not (a) a fully empty cherry-pick under rule 14 or (b) a bounded deferral recorded in the ledger with a named later target commit.
- You catch yourself reasoning toward "the applied commit no longer touches source paths after resolution, so it's no-build now," "I'll align the source paths to REFERENCE since REFERENCE has the final state anyway," "this source intent was already verified at the G7 [compilation] fold," "I'll fold the post-G7 REFERENCE state into the G7 fix so later cherry-picks land cleanly," "I'll align everything to REFERENCE except a PROTECTED set of paths," "I'll just run `cherry-pick -X ours` to get past the conflicts and clean up after," or "I'll fold idx N+k's restorations forward into idx N's build-fix" — these are HP-1 / HP-2 / HP-8 / rule 10 violations and Stop Conditions in their own right, regardless of how reasonable the framing sounds.
