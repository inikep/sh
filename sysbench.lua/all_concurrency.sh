...
dname=${13}
usepk=${14}
concurrency="${15} ${16} ${17} ${18} ${19} ${20}"

# warm the cache but results will wiped
echo point-query.pre
bash run.sh $ntabs $nrows $readsecs  $engine $setup 0        point-query.pre 100    $client $tableoptions $sysbdir $ddir $dname $usepk $concurrency
